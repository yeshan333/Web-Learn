...
def make_shell_context():
    return dict(db=db, Note=Note, Author=Author, Article=Article, Book=Book, Writer=Writer,
                Country=Country, Capital=Capital, Teacher=Teacher, Student=Student, Draft=Draft)
...
## 使用Flask-Migrate迁移数据库 pipenv install flask-migrate
### flask db init 创建迁移文件（migrations）
### flask db migrate -m "add note timestamp" 生成迁移脚本,注意检查一下迁移脚本
### flask db upgrade 更新数据库

# ---------------------------------------------------------------------------
# 事件监听
# Alchemy 装饰器 listens_for
